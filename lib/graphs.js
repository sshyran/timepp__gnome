...
var VBars = class {
    ...
    _draw_coord_system () {
        ...
        let y_label_font_desc = Pango.font_description_from_string(`bold ${this.coord_info.y_label_size}px`);

        let half_axes = this.coord_info.axes_width / 2;

        let a = this.coord_system_drawing_area.get_allocation_box();
        let x_axis_length = a.x2 - a.x1 - this.coord_info.x_offset - this.coord_info.axes_width;
        let y_axis_length = a.y2 - a.y1 - this.coord_info.y_offset - this.coord_info.axes_width;

        let y_max = Math.round(10 * (this.coord_info.y_max / this.coord_info.y_conversion_factor)) / 10;
        ...
        for (let i = 0; i < this.coord_info.n_rulers; i++) {
            ...
            let n = Math.round(10 * ruler_unit * (this.coord_info.n_rulers - i)) / 10;
            let pango_layout =
                this.coord_system_drawing_area.create_pango_layout(this.coord_info.y_label_format_func(n));
    ...
    _draw_vbars () {
        ...
        let a             = this.coord_system_drawing_area.get_allocation_box();
        let x_axis_length = a.x2 - a.x1 - this.coord_info.x_offset;
        let y_axis_length = a.y2 - a.y1 - this.coord_info.y_offset - this.coord_info.axes_width;
